...
- No personnel records beyond active certifications and shift assignments.
- No access to airline credentials, customer-facing systems, or revenue management.
- No write access to the AODB, crew system, or any system of record.
...
RampIQ operates per-station. A deployment at one hub is independent from a deployment at another hub. Multi-station benchmarking and network-level views are available in the executive interface but require separate deployments at each participating station.

### Flight scope

RampIQ models the ground-side turnaround. From block-on to block-off. Pre-arrival readiness and post-departure cleanup are out of scope.

### Crew scope

RampIQ tracks ramp crews — ground handlers, pushback operators, baggage handlers, cargo handlers, deicing operators, and equipment operators. It does not track flight crew, customer-service agents, or maintenance personnel.

### Decision scope

RampIQ recommends and forecasts. It does not decide. Every action that affects an aircraft, a crew assignment, an equipment dispatch, or a gate assignment requires explicit human confirmation in the interface.
...
- **No anonymous performance comparison.** Crew members cannot see rankings, leaderboards, or relative performance comparisons against peers.
- **No performance score that affects compensation, retention, or discipline.**
- **Visibility controls.** Each crew member can toggle which competency surfaces are visible to themselves, to their direct lead, and to station leadership.
- **Override transparency.** When a recommendation involves reassigning a crew member, the affected crew member sees the recommendation in their own app and can request a swap before accepting.
...
- All operational data is stored in the airline's choice of cloud region.
...
- Crew location data, where collected, is retained for 14 days and deleted on a rolling basis.
- An airline can request export and deletion of all data within 30 days of contract termination.
...
If the flight schedule feed, crew system, or BHS feed is unavailable, RampIQ continues operating on its last-known state and surfaces a clear "stale data" indicator across all relevant screens. Recommendations are paused after 5 minutes of stale primary data.

### Loss of RampIQ availability

If RampIQ itself is unavailable, ramp operations continue using the airline's existing tools and procedures. There is no scenario in which a RampIQ outage grounds the operation.

### Disagreement between RampIQ and human judgment

When a ramp lead overrides a recommendation, the override is logged with reason code and outcome. The ramp lead's authority is absolute; the system's role in disagreement is to learn, not to escalate.

---

## What RampIQ commits to in pilot deployment

- **Measurable lift target.** A pilot is structured around a measurable target metric agreed in advance. Typical targets: a 5-10% reduction in tarmac delay minutes, a 2-4% reduction in turnaround time on heavy-load operations.
- **Pilot duration.** 90 days minimum, 180 days recommended.
- **Single-station scope.** Pilot is one hub or one spoke station.
- **Read-only by default.** No write access to airline systems is provisioned during pilot.
- **Audit trail.** Every recommendation, every override, every action staged or executed is logged.
- **Right of withdrawal.** The airline may terminate the pilot at any time with 30 days notice.

---

## Out of scope for v1

- Multi-station coordination and network-wide optimization.
- Direct integration with the FAA NAS or air-traffic control feeds beyond commercial relays.
- Passenger-facing communications or rebooking decisions.
- Cargo specialty handling beyond hazmat class indication.
