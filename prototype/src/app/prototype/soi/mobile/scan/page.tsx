...
import { isZebra } from '@/lib/soi-types';
import type { QrTarget, ScanEvent } from '@/lib/soi-types';
...
export default function ScanPage() {
  ...
  useEffect(() => {
    ...
    if (!identity) {
      router.replace('/prototype/soi/mobile');
      return;
    }
  ...
  async function handleScanResult(decodedValue: string) {
    ...
    switch (target.target_type) {
      case 'GATE':
        router.push(`/prototype/soi/mobile/gate/${target.gate_id}?${params.toString()}`);
        break;
      case 'EQUIPMENT':
        router.push(`/prototype/soi/mobile/equipment/${target.equipment_id}?${params.toString()}`);
        break;
      case 'DISPATCH':
        router.push(`/prototype/soi/mobile/lt-dispatch?${params.toString()}`);
        break;
      default:
        // CHECKPOINT and others → generic report
        router.push(`/prototype/soi/mobile/report?${params.toString()}`);
        break;
    }
  }
