...
Usage:
    python3 -m sim.runner                          # run full day, emit state every minute
    python3 -m sim.runner --fast                   # 200x speed
    python3 -m sim.runner --snapshot               # produce one snapshot at 14:42
    python3 -m sim.runner --start 870 --stop 900   # run a specific window
    python3 -m sim.runner --output /path/to/state.json
    python3 -m sim.runner --quiet --snapshot       # suppress status prints
"""
import argparse
import json
...
def parse_args():
    ...
    p.add_argument("--snapshot", action="store_true",
                   help="Fast-forward to 14:42 and produce one snapshot")
    p.add_argument("--quiet",  action="store_true",
                   help="Suppress status prints")
    p.add_argument("--output", type=Path, default=None,
                   help="Path to write state JSON")
    return p.parse_args()
...
def _print_snapshot_summary(intel: dict):
    """Pretty-print a snapshot summary to the terminal."""
    print("\n" + "═" * 72)
